...
- 文档版本：v1.2（Python 版 + 分阶段开发流程）
- 变更记录：v1.0 初始 Go 版；v1.1 按用户决定改用 Python 实现，玩法规则、数据库与结算算法不变；v1.2 追加第 19 章分阶段开发流程
- 对应需求源：`Order.md`
...
```text
范围确认 → 配置/数据先行 → 领域类实现 → 持久化接入
→ 模拟器逻辑 → 前端交互 → 测试 → 阶段验收与归档
```

**流程铁律：**

1. 一个阶段只做该阶段范围内的功能，不提前实现后续内容；
2. 每个阶段结束时，程序必须**可以运行、可以存档、可以测试**；
3. 新增玩法优先“注册新类 + 新增配置”，不修改旧类签名；确需修改时同步更新测试；
4. 进入下一阶段前，上一阶段全部测试必须回归通过；
5. 每个阶段完成后执行一次存档备份，并建议打一个 git tag（`v0.1`、`v0.2`……）；
6. 任何阶段出现核心结算错误，先修复再进入下一阶段，不允许把问题带到后面。

### 19.2 Stage 0：项目初始化

**目标：** 目录、依赖、运行入口和测试环境可用。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | 创建虚拟环境并安装依赖 | `.venv/`、`requirements.txt`、`requirements-dev.txt` | `pip install -r requirements*.txt` 成功 |
| 2 | 建立目录骨架 | `mygame/` 包、`configs/`、`assets/`、`data/`、`tests/` | 目录与第 6 章一致 |
| 3 | 入口与参数解析 | `main.py`、`mygame/__main__.py` | `--help/--http/--gm/--config/--db` 可解析 |
| 4 | 日志与异常基类 | `mygame/app.py`、`logging` 配置、`ConfigError` | 错误格式带模块上下文 |
| 5 | SQLite 连接器骨架 | `mygame/store/sqlite.py` | 可打开内存库并执行 PRAGMA |
| 6 | 空测试骨架 | `tests/unit/`、`tests/integration/` | `python -m pytest` 返回 0 |
...
### 19.3 v0.1：世界骨架与“一步一月”闭环

**目标：** 创建玩家、生成地图、移动一格推进一个月、按日循环结算、老死判定、存档恢复。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | 时间系统 | `mygame/gametime/*` | day_index 与年/月/日互转单测通过 |
| 2 | 配置加载与校验 | `mygame/config/*`、`world.yaml`、`actions.yaml`、`realms.yaml` | 坏配置启动失败并指出位置 |
| 3 | 地图系统 | `mygame/world/*` | 64×64 地图生成，每格恰好属于一个 Region |
| 4 | 名称与 ID | `mygame/io_utils/`、`mygame/namegen.py`、`mygame/idgen.py`、三个名称表 | 随机姓名可用，ID 重启连续 |
| 5 | 人物基础 | `mygame/person/*` | 玩家创建成功，传记保留最近 5 条 |
| 6 | 动作框架 | `mygame/action/base.py`、`registry.py`、`move.py` | 动作可注册、可入队、按百分比推进 |
| 7 | 延寿字典与死亡 | `mygame/simulator/lifespan.py` | 延寿事件按生效日参与死亡判定，死亡最后判定 |
| 8 | 模拟器月结算 | `mygame/simulator/*` | `advance_one_step()` 按第 9 节顺序执行 |
| 9 | 全量数据库结构 | `mygame/store/*`、迁移 001（第 12 节 DDL） | 建表、备份、恢复可用 |
| 10 | 终端前端 | `mygame/frontend/*` | 可查看状态、10×10 地图、移动交互 |
| 11 | 测试 | 集成用例 1-6、10、11 | 全部通过 |

**本阶段结束时，玩家实际可以做到：**

```text
启动程序 → 看到自己的状态和地图 → 按方向键移动 1 格
→ 世界时间 +1 个月 → 本月 30 天结算日志输出 → 存档自动落盘
→ 关闭程序再启动 → 时间、位置、队列进度完全一致。
```
...
python -m pytest tests/ -q
python -m mygame
# 终端：查看状态 → 移动 → 世界+1月 → 退出 → 重启 → 状态与退出一致
```

### 19.4 v0.2：修炼与突破

**目标：** 玩家可以修炼增长修为、小层自动晋升、手动突破大境界、突破成功延寿、失败受罚。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | 修炼相关配置 | `realms.yaml`、`techniques.yaml`、`spirit_roots.yaml` | 引用与边界校验通过 |
| 2 | 修仙等级与灵根 | `mygame/cultivation/realm.py`、`linggen.py`、`technique.py` | 五行占比总和 100 |
| 3 | 修炼公式 | `mygame/cultivation/formula.py` | 灵气、功法、灵根、心情均参与计算 |
| 4 | 修炼动作 | `mygame/action/cultivate.py` | 30 天完成才结算，未满 100% 无收益 |
| 5 | 小层晋升 | `realm.py` 晋升逻辑 | 初期→中期→后期→圆满顺序正确 |
| 6 | 突破动作 | `mygame/action/breakthrough.py` | 成功/失败/冷却/轻伤按配置结算 |
| 7 | 突破与寿元联动 | 成功写正延寿事件，失败写负延寿事件 | 突破成功当天不会误死 |
| 8 | 状态展示 | 终端显示境界、小层、修为百分比 | 修炼后数值变化正确 |
| 9 | 测试 | 突破公式、失败惩罚、晋升、延寿用例 | 全部通过且 v0.1 测试回归通过 |

**本阶段结束时，玩家实际可以做到：**

```text
选择功法 → 入队修炼 → 移动推进一个月 → 修为增加
→ 修为满自动升小层 → 圆满后手动突破 → 成功升大境界并延寿 / 失败受罚
→ 剩余寿命随突破变化，死亡判定始终正确。
```
...
python -m pytest tests/ -q
python -m mygame
# 终端：入队修炼 → 移动 → 查看修为 → 圆满后突破 → 观察成功/失败结果
```

### 19.5 v0.3：动作扩展与生态

**目标：** 娱乐、情绪、狩猎、采集可玩；动物、植物、物品、灵石进入世界循环；地块事件触发。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | 物品与灵石 | `mygame/item/*`、`items.yaml` | 模板/实例/堆叠/动态词条正确 |
| 2 | 物品与灵石存取 | `repo_item.py` | 重启后背包与灵石一致 |
| 3 | 娱乐与情绪动作 | `entertain.py`、`emotion.py` | 心情/性格变化正确，情绪动作 0 天即时生效 |
| 4 | 动物系统 | `mygame/ecology/animal.py`、`animals.yaml`（20 种） | 每日随机移动 1 格、位置持久化 |
| 5 | 植物系统 | `mygame/ecology/plant.py`、`plants.yaml`（20 种） | 幼苗→成熟→枯萎，采集后按配置重生 |
| 6 | 狩猎动作 | `mygame/action/hunt.py` | 按境界与危险度判定，掉落正确 |
| 7 | 采集动作 | `mygame/action/gather.py` | 只能采成熟株，失败分支正确 |
| 8 | 地块/区域事件 | `mygame/event/template.py`、`events.yaml` | 到达新地块触发并写事件日志 |
| 9 | 前端展示 | 背包、灵石、地图动植物标记 | 终端可查看 |
| 10 | 测试 | 掉落、生长、重生、地块事件用例 | 全部通过且 v0.1/v0.2 回归通过 |

**本阶段结束时，玩家实际可以做到：**

```text
移动时看到地图上的动植物 → 狩猎/采集获得物品和灵石
→ 娱乐恢复心情 → 到达特殊地块触发事件 → 所有收获落库。
```
...
python -m pytest tests/ -q
python -m mygame
# 终端：查看背包 → 走到有动植物的格子 → 狩猎/采集 → 查看物品与灵石变化
```

### 19.6 v0.4：NPC 与 AI

**目标：** 世界里有 30–100 个 NPC，规则 AI 每月自主决策、修炼、突破、移动；DeepSeek 接口就绪但默认关闭。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | 性格生成与事件影响 | `mygame/personality/*` | 五维均在 0-100，事件可修改 |
| 2 | NPC 人口系统 | 初始生成 30 个、年末出生、上限 100、死亡移除 | 人口曲线符合配置 |
| 3 | 规则 AI | `mygame/ai/base.py`、`rule.py`、`ai.yaml` | NPC 可按性格权重选择动作并执行 |
| 4 | NPC 修炼与突破 | 复用 v0.2 动作，由 AI 入队 | NPC 可自主达到下一境界 |
| 5 | LLM 配置与提示词 | `llm.yaml`、`assets/prompts/*`、`mygame/llmclient/deepseek.py` | `enabled=false` 时零网络请求 |
| 6 | LLM 动作链 | `mygame/ai/llm.py` | 返回动作全部过 `can_execute()`，非法丢弃 |
| 7 | 突发状况响应 | 世界事件触发提案入口 | 提案校验通过才执行 |
| 8 | LLM 审计 | `llm_audit_log` 写入 | 请求/响应/校验结果可查 |
| 9 | 测试 | 固定种子 NPC 行为、人口、非法提案丢弃 | 全部通过且 v0.1-v0.3 回归通过 |

**本阶段结束时，世界会变成：**

```text
玩家每次移动一个月，NPC 们也在同一个月内决策和行动；
NPC 会修炼、突破、移动、死亡、出生；LLM 开关关闭时不产生任何网络调用。
```
...
python -m pytest tests/ -q
python -m mygame
# 终端：连续移动数月 → 查看事件日志 → 观察 NPC 出生/修炼/突破/死亡记录
```

### 19.7 v0.5：GM、HTTP 与收尾

**目标：** 提供完整 GM 权限、HTTP API、文档和最终验收。

| 顺序 | 工作内容 | 交付物 | 完成判据 |
|---|---|---|---|
| 1 | GM 终端菜单 | 第 13.2 节全部命令 | 修改有权限控制并写事件日志 |
| 2 | HTTP 状态与指令接口 | `mygame/server/http.py`、`/api/state`、`/api/command` | curl 可查询与下发指令 |
| 3 | HTTP GM 鉴权 | `X-Admin-Token` + 管理接口 | 无 token 拒绝 |
| 4 | 事件日志查询 | `list_events` 终端与 HTTP | 可按 scope 过滤 |
| 5 | 文档 | `README.md`、运行说明、配置说明 | 新人按文档可跑通 |
| 6 | 全量回归 | 全部单测 + 集成测试 | `python -m pytest tests/ -q` 全绿 |
| 7 | 备份恢复演练 | 备份、删档、恢复 | 恢复后数据一致 |
| 8 | 打包说明（可选） | PyInstaller 打包笔记 | 生成可分发目录或单文件 |
...
curl -X POST http://127.0.0.1:8080/api/command \
     ...
     -d '{"type":"add_spirit_stones","params":{"amount":100}}'
```

### 19.8 每个阶段的通用完成定义（DoD）

一个阶段只有全部满足以下条件，才算完成：

- [ ] 本阶段所有单元测试和集成测试通过；
- [ ] 之前所有阶段的测试回归通过；
- [ ] `python -m mygame` 可启动、可交互、可正常退出；
- [ ] 执行一次移动并重启后，世界时间、人物、队列、物品、生态状态一致；
- [ ] 所有新增数值均来自 YAML 配置，无硬编码；
- [ ] 所有关键变化写入 `event_log`；
- [ ] 无破坏旧类接口的修改（确需修改时，旧测试已同步更新）；
- [ ] 存档已完成一次备份；
- [ ] 建议：git 提交并打标签 `v0.x`。
...
| 阶段 | 命令 | 必须观察到的结果 |
|---|---|---|
| Stage 0 | `python main.py --help` | 显示全部参数 |
| v0.1 | `python -m mygame` 后移动 | 世界 +1 个月，重启状态一致 |
| v0.2 | 修炼→移动；突破 | 修为增长；突破成功/失败按规则结算 |
| v0.3 | 狩猎/采集 | 获得物品或灵石，失败分支正确 |
| v0.4 | 连续移动数月 | NPC 出生、修炼、突破、死亡事件出现 |
| v0.5 | `curl /api/state` | 返回 JSON 世界状态；GM 需 token |
...
```bash
git add -A
git commit -m "feat: v0.1 世界骨架与一步一月"
git tag v0.1

# 数据备份
cp data/save.db "data/backups/manual-v0.1-$(date +%Y%m%d-%H%M%S).db"
```

同时在 `SPEC.md` 的变更记录中追加一行，说明本阶段完成范围和遗留问题。
